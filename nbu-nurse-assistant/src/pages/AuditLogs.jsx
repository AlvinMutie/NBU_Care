...
import { ShieldCheck, FileText, Search, Filter, AlertTriangle, Clock, ChevronLeft, ChevronRight } from 'lucide-react';

export default function AuditLogs() {
  const logs = [
    { timestamp: '10 mins ago', user: 'Dr. Sarah (Consultant)', action: 'Updated ward guideline: GBS Prophylaxis', status: 'Approved', type: 'success' },
    { timestamp: '1 hour ago', user: 'Nurse Joy', action: 'Used the Emergency Calculator for Bed 4', status: 'Recorded', type: 'info' },
    { timestamp: '2 hours ago', user: 'System', action: 'Alert: Phototherapy dose limit reached on a student account', status: 'Warning', type: 'warning' },
    { timestamp: '3 hours ago', user: 'Nursing In-Charge', action: 'Removed access for temporary staff member (ID: 08922)', status: 'Done', type: 'success' },
    { timestamp: '5 hours ago', user: 'Student Kibet', action: 'Practised Clinical Math Scenario #3', status: 'Recorded', type: 'info' },
  ];
                ...
                <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right">
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold ${
                      ...
                      {log.type === 'warning' ? <AlertTriangle className="w-3.5 h-3.5"/> : <ShieldCheck className="w-3.5 h-3.5" />}
                      {log.status}
                    </span>
                  </td>
                </tr>
